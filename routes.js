...
router.get("/editar/:idInmueble", (req, res) => {
  ...
  fetch("http://api:3001/cp")
    ...
    .then((asentamientos) => {
      ...
      fetch(`http://api:3001/inmuebles/${req.params.idInmueble}`)
        .then((data) => data.json())
        .then((data) => {
          if (!data.results[0]) {
            res.redirect("/");
...
router.get("/perfil", (req, res) => {
  ...
  fetch(`http://api:3001/${ruta}/${req.session.correo}`)
    .then((data) => data.json())
    .then((data) => {
      if (data.err) {
        res.redirect("/");
...
router.get("/about", (req, res) => {
  fetch("http://api:3001/administradores")
    .then((data) => data.json())
    .then((data) => {
      if (data.err) {
...
router.get("/usuarios/:correo/:tipo", (req, res) => {
  ...
  fetch(`http://api:3001/${ruta}/${req.params.correo}`)
    .then((data) => data.json())
    .then((data) => {
      if (data.err) {
        res.redirect("/");
...
router.post("/login", (req, res) => {
  fetch(`http://api:3001/cuentas/${req.body.correo}`)
    ...
    .then((data) => {
      ...
      if (data.results[0].password == req.body.password) {
        ...
        fetch(`http://api:3001/${ruta}/${req.body.correo}`)
          .then((data) => (data.json ? data.json() : res.status(401).end()))
          .then((data) => {
            if (data.err) {
...
router.post("/nuevo", (req, res) => {
  ...
  fetch(`http://api:3001/Inmueble`, {
    ...
    .then((data) => {
      ...
      fetch(
        req.session.tipo == "Gerente"
          ? "http://api:3001/gerenteProyecto"
          : "http://api:3001/agenteInmueble",
        {
          method: "POST",
